    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try (PrintWriter out = resp.getWriter()) {
            ...
            if (email != null && memberJDBC.isEmailExists(email)) {
                ...
                jsonResponse.addProperty("message", "新密碼已寄至您的電子郵件，請查收。");
                System.out.print("確認為會員，並已送出新密碼");
            } else {
                jsonResponse.addProperty("status", "error");
                jsonResponse.addProperty("message", "電子郵件未註冊或輸入無效。");
                System.out.print("非會員，請註冊");
            }
            ...
            e.printStackTrace();
            jsonResponse.addProperty("status", "error");
            jsonResponse.addProperty("message", "讀取數據錯誤，請稍後再試。");
            System.out.print("數據錯誤");
        } catch (Exception e) {
            e.printStackTrace();
            jsonResponse.addProperty("status", "error");
            jsonResponse.addProperty("message", "系統錯誤，請稍後再試。");
